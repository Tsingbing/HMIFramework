...
import HMI.FlightMap             1.0
import HMI.ScreenTools           1.0
//import HMI.VehicleManager   1.0
import HMI.Vehicle               1.0
//import HMI.QGCPositionManager    1.0
...
Map {
    ...
    property var    gcsPosition:                    HMI.qgcPositionManger.gcsPosition
    property real   gcsHeading:                     HMI.qgcPositionManger.gcsHeading
    ...
    function _possiblyCenterToVehiclePosition() {
        if (!firstVehiclePositionReceived && allowVehicleLocationCenter && activeVehicleCoordinate.isValid) {
            ...
            zoomLevel = HMI.flightMapInitialZoom
        }
    }
...
//    }
    ...
    onGcsPositionChanged: {
        if (gcsPosition.isValid && allowGCSLocationCenter && !firstGCSPositionReceived && !firstVehiclePositionReceived) {
            ...
            if(HMI.settingsManager.flyViewSettings.keepMapCenteredOnVehicle.rawValue || !activeVehicleCoordinate.isValid)
                center = gcsPosition
        }
    }
    ...
    function updateActiveMapType() {
        var settings =  HMI.settingsManager.flightMapSettings
    ...
    Connections {
        target:             HMI.settingsManager.flightMapSettings.mapType
        onRawValueChanged:  updateActiveMapType()
    }

    Connections {
        target:             HMI.settingsManager.flightMapSettings.mapProvider
        onRawValueChanged:  updateActiveMapType()
    }
